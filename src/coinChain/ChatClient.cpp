        ...
        switch (_mode) {
            ...
            case wait_for_motd: {
                if (rx.find(" 004 ") != string::npos) {
                    ...
                    log_warn("IRC name already in use\n");
                    _name_in_use = true;
                    // close and rejoin
                    _socket.close();
                    _mode = wait_for_notice;
                ...
                if (rx.find(" 302 ") != string::npos) {
                    ...
                    if (words.size() > 3) {
                        ...
                        if (str.rfind("@") != string::npos) {
                            ...
                            log_warn("GetIPFromIRC() got userhost %s\n", host.c_str());
                ...
                if (name[0] == 'u') {
                    ...
                    if (decodeAddress(name, endpoint)) {
                        ...
                        if (_endpointPool.addEndpoint(endpoint, 51 * 60)) {
                            log_warn("IRC got new address: %s\n", endpoint.toString().c_str());
                            _notifier();
                        }
                        //                        nGotIREndpointes++;
                    }
                    else {
                        log_warn("IRC decode failed\n");
                    }
                }
                break;
            }
